...
class YellowPodcastfeed
{
	...
	{
		$this->yellow = $yellow;
		$this->yellow->config->setDefault("podcastLocation", "/podcastfeed/");
		$this->yellow->config->setDefault("podcastFileXml", "podcastfeed.xml");
		$this->yellow->config->setDefault("podcastFilter", "podcasts");
		...
		$this->yellow->config->setDefault("podcastSubcategory", "Podcasts");
	}

	// Handle page parsing
	function onParsePage()
	{
		if($this->yellow->page->get("template")=="podcastfeed")
		{
			...
			$chronologicalOrder = ($this->yellow->config->get("podcastFilter")!="podcasts");
			if($this->isRequestXml())
			{
				...
				$output .= "<rss version=\"2.0\" xmlns:googleplay=\"http://www.google.com/schemas/play-podcasts/1.0\" xmlns:itunes=\"http://www.itunes.com/dtds/podcast-1.0.dtd\" xmlns:content=\"http://purl.org/rss/1.0/modules/content/\" xmlns:dc=\"http://purl.org/dc/elements/1.1/\" xmlns:atom=\"http://www.w3.org/2005/Atom\">\r\n";
				$output .= "<channel>\r\n";
				...
				if($this->yellow->config->get("podcastImageUrl")) {
					$output .= "<image>\r\n";
					$output .= "<url>".$this->yellow->page->scheme."://".$this->yellow->page->address.$this->yellow->page->base.$this->yellow->config->get("podcastImageUrl")."</url>\r\n";
				...
				if($this->yellow->config->get("podcastImageUrl")) $output .= "<itunes:image href=\"".$this->yellow->page->scheme."://".$this->yellow->page->address.$this->yellow->page->base.$this->yellow->config->get("podcastImageUrl")."\" />\r\n";
	...
	function onExtra($name)
	{
		...
		if($name=="header")
		{
			...
			$tag = $_REQUEST["tag"];
			$locationPodcastfeed = $this->yellow->config->get("serverBase").$this->yellow->config->get("podcastLocation");
			if($tag) $locationPodcastfeed .= $this->yellow->toolbox->normaliseArgs("tag:$tag", true);
			$locationPodcastfeed .= $this->yellow->toolbox->normaliseArgs("$pagination:".$this->yellow->config->get("podcastFileXml"), false);
			$output = "<link rel=\"alternate\" type=\"application/rss+xml\" href=\"$locationPodcastfeed\" />\n";
		}
		return $output;
	}
...
$yellow->plugins->register("podcastfeed", "YellowPodcastfeed", YellowPodcastfeed::VERSION);
?>
